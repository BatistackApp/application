            ...
            <div class="client-info">
                ...
                <div class="info-block">
                    ...
                    @if($document->client->addresses->isNotEmpty())
                        ...
                        {{ $adresse->street }}<br>
                        {{ $adresse->postal_code }} {{ $adresse->city }}<br>
                    @endif
                    @if($document->client->tva_number)
                        TVA : {{ $document->client->tva_number }}
                    @endif
                </div>
            </div>
            ...
            <div class="document-info">
                ...
                <div class="info-block">
                    ...
                    <div class="info-row">
                        ...
                        <span class="value">{{ $document->date_echeance?->format('d/m/Y') ?? 'Non définie' }}</span>
                    </div>
                    ...
                    @endif
                    @if($document->avancement_pct)
                        <div class="info-row">
                            <span class="label">Avancement :</span>
                            <span class="value">{{ number_format($document->avancement_pct, 1, ',', ' ') }} %</span>
                        </div>
                    @endif
                </div>
            </div>
    ...
    <div class="document-totals">
        <table>
            ...
            @foreach($totaux['par_taux_tva'] as $tva)
                <tr>
                    <td>TVA {{ number_format($tva['taux'], 1, ',', ' ') }} % sur {{ number_format($tva['base_ht'], 2, ',', ' ') }} €</td>
...
@section('styles')
    {{-- Mêmes styles que devis.blade.php --}}
    <style>
        .document-header {
            margin-bottom: 2rem;
        }
